...
public class UnosAutoController {
	private Logger log =  LoggerFactory.getLogger(UnosAutoController.class);
	...
	public void unesi() {
		...
		if (snagaFld.getText().isBlank())
			notice += "Snaga je obavezan podatak\n";
		...
		BigDecimal tmpSnaga = new BigDecimal(snagaFld.getText());
		Stanje tmpStanje;
		...
		Automobil auto = new Automobil(tmpNaslov, tmpOpis, tmpSnaga, tmpStanje, tmpCijena, -1);
		try{	
			BazaPodataka.pohraniNoviAuto(auto);
		...
		alert.setHeaderText("");
		alert.setContentText("Uspjesno unesen automobil\n");
		alert.showAndWait();
	}
